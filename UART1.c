...
void UART1_Init(void){
  ...
  UART1->GPRCM.RSTCTL = 0xB1000003; // reset UART1
  UART1->GPRCM.PWREN = 0x26000001; // activate UART1
  Clock_Delay(24);
  IOMUX->SECCFG.PINCM[PA8INDEX]  = 0x00000082;
  UART1->CLKSEL = 0x08; // bus clock KEPT THESE NEXT 4 LINES SAME, JUST CHANGED UART0 TO 1
  UART1->CLKDIV = 0x00; // no divide
  UART1->CTL0 &= ~0x01; // disable UART1
  UART1->CTL0 = 0x00020018; // enable fifos, tx only
  // 40,000,000/16 = 2,500,000 --- 2,500,000/2000 = 1250
  UART1->IBRD = 1250; 
  UART1->FBRD = 0;
