...
void TIMG12_IRQHandler(void){uint32_t pos,msg;
  if((TIMG12->CPU_INT.IIDX) == 1){ // this will acknowledge
   GPIOB->DOUTTGL31_0 = GREEN; // toggle PB27 (minimally intrusive debugging)
    ...
    UART1_OutChar(0x3C);               // start byte
    UART1_OutChar('0'+(pos/1000)%10);  // ones digit
    UART1_OutChar('0'+(pos/100)%10);   // tenths digit
    UART1_OutChar('0'+(pos/10)%10);    // hundredths digit
...
int main(void){ // main5
  ...
  TimerG12_IntArm(2666667, 2); // 80MHz/2666667 = 30Hz
 ...
 // complete this
    ...
    do{
        d2 = UART2_InChar();
    }while(d2 == 0); // wait until we actually get something
    char d3;
    do{
        d3 = UART2_InChar();
    }while(d3 == 0); // wait until we actually get something
    char d4;
    do{
        d4 = UART2_InChar();
    }while(d4 == 0); // wait until we actually get something
 ...
 // calculate PositionReceived from message
    PositionReceived = (d2-'0')*1000 + (d3-'0')*100 + (d4-'0')*10;
    OutFix(PositionReceived);
